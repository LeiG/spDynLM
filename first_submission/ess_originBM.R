## apply stopping rule with proposed ESS calculation with original BM estimates
require(mcmcse)
...
while(1){
  ...
  ess.origin<- (std/mcse)^2
  write.table(ess.origin, paste(thres, "ess_origin.txt", sep="_"))
  if(all(ess.origin>=thres)){
    #     ess.old<- apply(X, 2, ess)
    ess.old<- apply(samples[0:check,], 2, ess)
    ess.app<- 4*(z/eps)^2
    #     X.mean<- apply(X, 2, mean)
    X.mean<- apply(samples[0:check,], 2, mean)
    out<- list(n= check, app= ess.app, old= ess.old, new= ess.origin)
    write.table(out, paste(thres, "output_ess_origin.txt", sep="_"))
    write.table(mcse, paste(thres, "mcse_ess_origin.txt", sep="_"))
    write.table(std, paste(thres, "sd_ess_origin.txt", sep="_"))
    write.table(X.mean, paste(thres, "mean_ess_origin.txt", sep="_"))
    break
  }
